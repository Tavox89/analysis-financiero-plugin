...
final class OrderSettlementBatchesRepository extends BaseRepository {
	protected $table_key = 'order_settlement_batches';

	public function create_batch( array $data ) {
		if ( ! $this->has_table() ) {
			return $this->error( 'asdl_fin_order_settlement_batches_missing', 'La tabla de lotes de abonos aun no esta disponible.' );
		}

		$payload = array(
			'batch_key'          => sanitize_text_field( (string) ( $data['batch_key'] ?? '' ) ),
			'contact_id'         => absint( $data['contact_id'] ?? 0 ),
			'origin'             => sanitize_key( (string) ( $data['origin'] ?? 'profile_settlement' ) ),
			'mode'               => sanitize_key( (string) ( $data['mode'] ?? 'standard' ) ),
			...
			'total_received'     => round( (float) ( $data['total_received'] ?? 0 ), 6 ),
			'total_covered'      => round( (float) ( $data['total_covered'] ?? 0 ), 6 ),
			'discount_total'     => round( (float) ( $data['discount_total'] ?? 0 ), 6 ),
		...
		$result = $this->db()->insert(
			$this->table(),
			$payload,
			array( '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%f', '%f', '%f', '%d', '%d', '%f', '%d', '%s', '%s', '%s' )
		);

		if ( false === $result ) {
			return $this->error( 'asdl_fin_order_settlement_batch_insert', 'No se pudo registrar el lote del abono.' );
		}
	...
	public function update_batch( $batch_id, array $data ) {
		...
		foreach ( array( 'origin', 'mode', 'status', 'currency', 'method_key', 'preview_signature' ) as $text_key ) {
			if ( ! array_key_exists( $text_key, $data ) ) {
				continue;
			}

			$payload[ $text_key ] = in_array( $text_key, array( 'origin', 'mode', 'status', 'method_key' ), true )
				? sanitize_key( (string) $data[ $text_key ] )
				: sanitize_text_field( (string) $data[ $text_key ] );
			if ( 'currency' === $text_key ) {
				$payload[ $text_key ] = strtoupper( $payload[ $text_key ] );
			}
			$formats[] = '%s';
		}

		foreach ( array( 'contact_id', 'item_count', 'processed_count' ) as $int_key ) {
		...
		foreach ( array( 'total_received', 'total_covered', 'discount_total', 'processed_total' ) as $float_key ) {
	...
	public function append_items( $batch_id, array $items ) {
		$table = Tables::name( 'order_settlement_batch_items' );
		...
		foreach ( $items as $item ) {
			...
			$result = $this->db()->insert(
				$table,
				$payload,
				array( '%d', '%d', '%s', '%s', '%d', '%d', '%s', '%s', '%f', '%f', '%f', '%f', '%f', '%s', '%s', '%s', '%s', '%s' )
			);
	...
	public function update_item( $item_id, array $data ) {
		...
		foreach ( array( 'balance_before', 'cover_amount', 'customer_paid_amount', 'discount_amount', 'expected_balance_after' ) as $float_key ) {
			if ( ! array_key_exists( $float_key, $data ) ) {
				continue;
			}
	...
	public function find_active_for_contact( $contact_id, $origin = 'profile_settlement' ) {
		if ( ! $this->has_table() ) {
			return null;
		}
	...
	public function list_batch_errors( $batch_id, $limit = 50 ) {
		return $this->list_items_by_status( $batch_id, array( 'error' ), $limit );
	}
	...
	private function normalize_batch_row( array $row ) {
		...
		foreach ( array( 'total_received', 'total_covered', 'discount_total', 'processed_total' ) as $key ) {
	...
	private function normalize_item_row( array $row ) {
		...
		foreach ( array( 'balance_before', 'cover_amount', 'customer_paid_amount', 'discount_amount', 'expected_balance_after' ) as $key ) {
			$row[ $key ] = round( (float) ( $row[ $key ] ?? 0 ), 6 );
		}
